...
import {players, addPlayer, removePlayer, playerForId, updatePlayerData} from "./players.js";
...
export default (io) => {
  ...
  io.on('connection', socket => {
    ...
    socket.on('client-credentials', async creds => {
        ...
        if (!user)
            console.error('Username incorrect for multiplayer!');

        addPlayer(socket.id, user);
        let player = playerForId(socket.id);
        ...
        socket.on('updatePosition', data => {
            let newData = updatePlayerData(data);
        ...
        socket.on('inventory-update', async dat => {
            let user = await dbFindUser(dat.user);
            if (user) {
        ...
        socket.on('map-update', async dat => {
            let user = await db.dbFindUser(dat.user);
            if (user) {
                user.map = dat.map;
                user.save();
