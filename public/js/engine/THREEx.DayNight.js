...
THREEx.DayNight.SunLight = function () {
    ...
    this.update = function (sunAngle) {
        ...
        if (phase === 'day') {
            ...
            Alight.color.set("rgb(" + (Math.floor(Math.sin(sunAngle) * 70)) + "," + (Math.floor(Math.sin(sunAngle) * 70)) + "," + (Math.floor(Math.sin(sunAngle) * 70)) + ")");
        }
        else {
            light.intensity = 0;
            Alight.color.set(0x000000);
        }
    }
}
...
THREEx.DayNight.Skydom = function () {
    var geometry = new THREE.SphereGeometry(500, 32, 15);
